package org.exam.mapper;

import lombok.extern.slf4j.Slf4j;
import org.exam.ExamServerApplication;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.test.context.junit4.SpringRunner;

/**
 * @author heshiyuan
 * @description <p></p>
 * @path exam-server/org.exam.mapper
 * @date 15/03/2019 21:03
 * @github http://github.com/shiyuan2he
 * @email shiyuan4work@126.com
 * Copyright (c) 2019 shiyuan4work@126.com All rights reserved.
 * @price ¥5    微信：hewei1109
 */
@Slf4j
@RunWith(SpringRunner.class)
@SpringBootTest(classes = ExamServerApplication.class)
public class TMenuMapperTest {
    @Autowired
    TMenuMapper tMenuMapper;
    @Test
    public void countByExample() {
    }

    @Test
    public void deleteByExample() {
    }

    @Test
    public void deleteByPrimaryKey() {
    }

    @Test
    public void insert() {
    }

    @Test
    public void insertSelective() {
    }

    @Test
    public void selectByExample() {
    }

    @Test
    public void selectByPrimaryKey() {
    }

    @Test
    public void updateByExampleSelective() {
    }

    @Test
    public void updateByExample() {
    }

    @Test
    public void updateByPrimaryKeySelective() {
    }

    @Test
    public void updateByPrimaryKey() {
    }

    @Test
    public void getAllMenu() {
    }

    @Test
    public void getMenusByUserId() {
    }

    @Test
    public void getMenusByParentId() {
        log.info("{}", tMenuMapper.getMenusByParentId(0));
    }
}